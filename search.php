...
{
	// Insert the form
	$OutputBuffers[$OUTPUT_FORM_SEARCHBOX] = $STR_FORM_SEARCHFOR . " <input type=\"text\" name=\"zoom_query\" size=\"35\" value=\"".htmlspecialchars($query)."\" id=\"zoom_searchbox\" class=\"zoom_searchbox\" autocorrect=\"off\" autocapitalize=\"off\" />\n";
	$OutputBuffers[$OUTPUT_FORM_SEARCHBUTTON] = "&nbsp;&nbsp;<input type=\"submit\" value=\"" . $STR_FORM_SUBMIT_BUTTON . "\" class=\"zoom_button\" />\n";
	if ($FormFormat == 2)
	{
		...
		if ($UseCats)
		{
			...
			if ($SearchMultiCats)
			{
				...
				$OutputBuffers[$OUTPUT_FORM_CATEGORIES] .= "</ul><br />\n";
			}
			else
			{
...
//Print heading
$OutputBuffers[$OUTPUT_HEADING] .= "<div class=\"searchheading\">" . $STR_RESULTS_FOR . " <i>" . $queryForHTML;
if ($UseCats)
{
	if ($cat[0] == -1)
	{
...
$OutputBuffers[$OUTPUT_HEADING] .= "</i><br /></div>\n";
...
if ($IsMaxLimitExceeded)
	$OutputBuffers[$OUTPUT_SUMMARY] .= $STR_PHRASE_CONTAINS_COMMON_WORDS . "<br />";
...
if ($num_pages > 1)
	$OutputBuffers[$OUTPUT_PAGESCOUNT] .= "<div class=\"result_pagescount\">" . $num_pages . " " . $STR_PAGES_OF_RESULTS . "</div>\n";
...
if ($num_pages > 1)
{
	...
	$OutputBuffers[$OUTPUT_PAGENUMBERS] .= "<div class=\"result_pages\"><b>\n" . $STR_RESULT_PAGES . "</b> ";
	if ($page > 1)
...
if ($Timing == 1 || $Logging == 1)
{
	...
	if ($Timing == 1)
		$OutputBuffers[$OUTPUT_SEARCHTIME] .= "<div class=\"searchtime\"><br />" . $STR_SEARCH_TOOK . " " . $timetaken . " " . $STR_SECONDS . "</div>\n";
}
